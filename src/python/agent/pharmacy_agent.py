...
in the FHIR Agents platform. It is the most safety-critical agent in
the system — its decisions directly affect what drugs a patient should or
should not receive.

Responsibilities:
  1. Fetch the patient's current medications and known allergies from the FHIR server
  ...
  6. Write a FHIR MedicationRequest to the FHIR server when a new medication is proposed
...
from config import LLM_MODEL, TEMP_PHARMACY, GROQ_API_KEY, GROQ_BASE_URL
