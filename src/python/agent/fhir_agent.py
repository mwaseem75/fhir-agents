"""
fhir_agent.py — FHIR Server Agent
=======================================
A LangChain agent that provides a natural language interface to any FHIR R4
server (configured via FHIR_BASE_URL).
...
— which shouldn't be required to ask "which patients are on Warfarin and have
a penicillin allergy?"

Who uses this agent:
  - Clinicians wanting a quick population overview without opening an EHR
  - Developers exploring what data is available on their FHIR server
...
Tools provided (10 total):
  ...
  search_clinical_guidelines  — RAG search against 50 clinical guidelines
...
from config import FHIR_BASE, FHIR_AUTH, FHIR_HEADERS, LLM_MODEL, TEMP_SPECIALIST, GROQ_API_KEY, GROQ_BASE_URL
...
def get_patient_list(query_params: str = "") -> str:
    """
    Retrieve a list of patients from the FHIR server.
...
def get_conditions_for_patient(patient_id: str) -> str:
    """
    Get all active medical conditions for a specific patient from the FHIR server.
    ...
    try:
        ...
        for e in entries:
            ...
            # Prefer coding display, fall back to code.text (some servers use text only)
...
def get_medications_for_patient(patient_id: str) -> str:
    """
    Get all active medications for a specific patient from the FHIR server.
    patient_id: The FHIR patient ID e.g. 'pt-001'
...
def get_observations_for_patient(patient_id: str, category: str = "") -> str:
    """
    Get clinical observations (labs, vitals) for a patient from the FHIR server.
...
def get_allergies_for_patient(patient_id: str) -> str:
    """
    Get all allergies and intolerances for a patient from the FHIR server.
    patient_id: The FHIR patient ID
...
def get_procedures_for_patient(patient_id: str) -> str:
    """
    Get all procedures performed for a patient from the FHIR server.
    patient_id: The FHIR patient ID
...
def search_patients_by_condition(condition_name: str) -> str:
    """
    Find all patients who have a specific condition on the FHIR server.
    condition_name: Condition to search for e.g. 'diabetes', 'hypertension', 'asthma'
    Returns list of patients with that condition.
    """
    try:
        # Fetch all conditions and filter client-side — many FHIR R4 servers don't support
...
def get_fhir_statistics() -> str:
    """
    Get comprehensive statistics about the FHIR server.
    ...
    try:
        ...
        result  = "FHIR R4 Server Statistics:\n"
        result += "─" * 40 + "\n"
...
SYSTEM_PROMPT = """You are the FHIR Agent — an intelligent AI assistant with direct access to a FHIR R4 server.

You are the bridge between natural language and the FHIR server. You help clinicians, developers, and analysts explore and understand FHIR data without needing to know FHIR query syntax.
...
4. RAG GUIDELINES — Enrich findings with clinical guidelines from the knowledge base
5. SERVER STATISTICS — Report on FHIR server content and health
6. CAPABILITY STATEMENT — Summarize and explain what the FHIR server supports,
   including all resource types, interactions, search parameters, and operations
7. RESOURCE DETAILS — Deep-dive into any specific FHIR resource type capabilities
