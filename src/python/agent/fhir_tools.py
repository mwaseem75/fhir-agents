...
the configured FHIR R4 server. Every FHIR read and write in the
...
def fhir_get(path: str) -> dict:
    ...
    The Accept header is required — without it some FHIR servers return XML,
    which none of the parsing code downstream is prepared to handle. The
    10-second timeout is generous enough for a cold-start query but short
    enough that a network partition doesn't stall an agent indefinitely.
    """
...
def fhir_post(path: str, data: dict) -> dict:
    ...
    Most FHIR servers return the created resource in the response body with
    the server-assigned id. We handle an empty body gracefully — some servers
    return 201 with no body on successful creation, which would cause
    response.json() to raise a decode error.

    The 30-second timeout is higher than for reads because FHIR writes
    can trigger business rule evaluation and indexing on the server.
    """
    url = f"{FHIR_BASE}/{path}"
    response = httpx.post(
        url,
...
# ═══════════════════════════════════════════════════════════════════════════════
#  READ TOOLS — fetch patient data from the FHIR server
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  WRITE TOOLS — create FHIR resources on the FHIR server
# ═══════════════════════════════════════════════════════════════════════════════
...
# This ensures Observations written to the FHIR server are properly coded rather than
